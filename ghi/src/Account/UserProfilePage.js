...
const UP = () => {
    ...
    const likeUser = async () => {
        ...
        const matchedUser = userId;  // userId comes from useParams
        const mutual = false; // assuming the like is not mutual by default
        const matchTimestamp = new Date().toISOString(); // current timestamp as an example

        const response = await fetch(`http://localhost:8000/likes/${loggedInUser}/${matchedUser}`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json',
